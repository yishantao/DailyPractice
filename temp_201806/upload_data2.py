...
class UploadData(object):
    ...
    def connect_database(self):
        db_pool = adbapi.ConnectionPool('pymysql', host=self.host, db=self.db, user=self.user, passwd=self.passwd,
                                        charset='utf8')
        return db_pool

    def create_table(self, tx):
        sql = '''
    ...
    def insert_table(self, tx, series):
    ...
    def run(self):
        database_pool = self.connect_database()
        database_pool.runInteraction(self.create_table)
        data = self.read_excel('student_num.xlsx')
        database_pool.runInteraction(self.insert_table, data['S_NUM'])
...
if __name__ == '__main__':
    a = UploadData(db='test', host='localhost', user='root', passwd='777888')
    a.run()
    reactor.callLater(2, reactor.stop)
    reactor.run()
